...
from config import spectrogramsPath, slicesPath, slicesInputPath, spectrogramsInputPath
...
def createSlicesFromInputSpectrograms(desiredSize):
	for filename in os.listdir(spectrogramsInputPath):
		if filename.endswith(".png"):
			sliceInputSpectrogram(filename,desiredSize)

def sliceInputSpectrogram(filename, desiredSize):
	genre = filename.split("_")[0] 	#Ex. Dubstep_19.png

	# Load the full spectrogram
	img = Image.open(spectrogramsInputPath+filename)
	...
	sliceInputPath = slicesInputPath+"{}/".format(genre);
	if not os.path.exists(os.path.dirname(sliceInputPath)):
		try:
			os.makedirs(os.path.dirname(sliceInputPath))
	...
	for i in range(nbSamples):
		...
		imgTmp.save(slicesInputPath+"{}/{}_{}.png".format(genre,filename[:-4],i))
